...
let projectsCustomersJoinAvailable = false;
...
async function detectProjectsCustomersJoin() {
  const cacheKey = 'projects.customers_join';
  const cached = readSchemaColumnCache();
  if (cacheKey in cached) {
    projectsCustomersJoinAvailable = cached[cacheKey];
    return projectsCustomersJoinAvailable;
  }

  const probe = await supabase
    .from('projects')
    .select('project_id, customers(name)')
    .limit(1);

  projectsCustomersJoinAvailable = !probe.error;
  writeSchemaColumnCache('projects', 'customers_join', projectsCustomersJoinAvailable);
  return projectsCustomersJoinAvailable;
}
...
export async function fetchAppData() {
  ...
  await detectProjectsCustomersJoin();
  await detectAccessRoleColumn();
  await detectUserProfileColumns();

  const projectSelect = projectsCustomersJoinAvailable
    ? '*, customers(name)'
    : '*';
...
async function resolveCustomerId(customerName, ownerUserId) {
  ...
  // New / slim DBs may not have a customers table — skip instead of 400/404.
  const customersProbe = await supabase.from('customers').select('customer_id').limit(1);
  if (customersProbe.error) {
    console.warn('[Supabase] Bỏ qua customers:', customersProbe.error.message);
    return null;
  }
