...
function taskToNode(task) {
  ...
  const status = completion?.completedAt ? 'done' : mapDbStatus(task.status);
  return {
    id: task.task_id,
    name: task.name,
...
function featureToNode(feature, tasks) {
  ...
  applyCompletionToNode(node, completionFromBlocks(feature.content_blocks), children);
  return node;
}
...
async function detectSubtaskColumn() {
  const { error } = await supabase.from('tasks').select('parent_task_id').limit(1);
  subtaskColumnAvailable = !error;
...
async function detectFeaturesContentBlocksColumn() {
  const { error } = await supabase.from('features').select('content_blocks').limit(1);
  featuresContentBlocksAvailable = !error;
  return featuresContentBlocksAvailable;
}

async function detectProjectsDocumentsColumn() {
  const { error } = await supabase.from('projects').select('documents').limit(1);
  projectsDocumentsAvailable = !error;
...
export async function fetchAppData() {
  ...
  const [projectsRes, featuresRes, tasksRes, usersRes] = await Promise.all([
    ...
    supabase.from('users').select('user_id,full_name,email,role,department,status,avatar_url').order('full_name', { ascending: true }),
  ]);
...
export async function createProduct({ name, customerName, deadline, ownerUserId }) {
  const trimmed = (name || '').trim();
  if (!trimmed) throw new Error('Vui lòng nhập tên sản phẩm');
...
export async function createChildNode(parentNode, { name, deadline, assignees }) {
  ...
  if (table === 'projects') {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        status: 'pending',
        ...(deadlineDb ? { deadline: deadlineDb } : {}),
        ...(assigneeList.length ? { content_blocks: { assignees: assigneeList } } : {}),
      })
      .select()
      .single();
    if (error) throw error;
  ...
  if (table === 'features') {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        ...(assigneeBlocks ? { content_blocks: assigneeBlocks } : {}),
      })
      .select()
      .single();
    if (error) throw error;
...
export async function saveNodePatch(node, patch) {
  ...
  if ('status' in patch) payload.status = mapUiStatus(patch.status);
...
export async function savePersonProfile(personId, { name, role, dept, status }) {
