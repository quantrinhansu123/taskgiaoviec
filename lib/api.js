...
function userSelectColumns() {
  const base = 'user_id,full_name,email,role,avatar_url';
  return [
    base,
...
export async function authenticatePersonLogin({ phone, password }) {
  ...
  const selectColumns = [
    ...
    'username',
    'email',
    userPasswordColumnAvailable ? 'password' : '',
    'role',
    'status',
    accessRoleColumnAvailable ? 'access_role' : '',
  ].filter(Boolean).join(',');
  ...
  if (userPhoneColumnAvailable) {
    query = query.or(`phone.eq.${loginPhone},username.eq.${loginPhone},email.eq.${loginPhone},user_id.eq.${loginPhone}`);
  } else {
    query = query.or(`username.eq.${loginPhone},email.eq.${loginPhone},user_id.eq.${loginPhone}`);
  }

  const { data, error } = await query.maybeSingle();
  if (error) throw error;
  const storedPassword = String(data?.password || '').trim();
  if (!data || (storedPassword && storedPassword !== loginPassword)) {
