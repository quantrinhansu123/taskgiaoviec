...
async function detectProjectsContentBlocksColumn() {
  const { error } = await supabase.from('projects').select('content_blocks').limit(1);
  projectsContentBlocksAvailable = !error;
  return projectsContentBlocksAvailable;
}

async function detectAccessRoleColumn() {
  const { error } = await supabase.from('users').select('access_role').limit(1);
  accessRoleColumnAvailable = !error;
...
function userSelectColumns() {
  ...
  return accessRoleColumnAvailable ? `${base},access_role` : base;
}
...
export async function savePersonProfile(personId, { name, role, dept, status, accessRole }) {
