...
export function normalizeWorkAction(raw) {
  ...
  if (kind === 'note' || kind === 'evaluation') {
    ...
    const evaluationType = (raw.evaluation_type || raw.evaluationType || '').trim();
    return {
      id: raw.id || newWorkActionId(),
      kind,
      title: title || content.split('\n')[0].slice(0, 120),
      note: content,
      ...(kind === 'evaluation' && evaluationType ? { evaluationType } : {}),
      startedAt: stamp,
...
export function workActionsToDb(actions) {
  return (actions || []).map((a) => ({
    ...
    evaluation_type: normalizeWorkActionKind(a.kind) === 'evaluation'
      ? ((a.evaluationType || '').trim() || null)
      : null,
...
export function formatWorkActionRange(action) {
  ...
  const endStr = sameDay
    ? end.toLocaleTimeString(localeDateTag(), { hour: '2-digit', minute: '2-digit', hour12: false })
...
export function formatDurationMinutes(mins) {
  ...
  if (mins < 60) return en ? `${mins} min` : `${mins} phút`;
