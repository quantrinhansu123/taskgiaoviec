...
export function collectTeamAssignments(products = []) {
  const rows = [];
...
export function buildGanttData(products, people, rangeStart, rangeEnd, teamFilter = null) {
  const teams = teamsFromPeople(people);
  const days = dateRangeKeys(rangeStart, rangeEnd);
  const assignments = collectTeamAssignments(products);
  const personTeam = (userId) => people.find((p) => p.id === userId)?.dept || 'Chung';
  const projectById = new Map((products || []).map((p) => [p.id, p]));
  ...
  const enrichBar = (bar) => {
    ...
    return {
      ...
      progress: barFillPercent(bar, project),
    };
  };
...
export function buildScheduleKpis(products = []) {
  const total = products.length;
  const doing = products.filter((p) => p.status === 'doing').length;
  const todo = products.filter((p) => p.status === 'todo').length;
  const done = products.filter((p) => p.status === 'done').length;
  const fail = products.filter((p) => p.status === 'fail').length;
