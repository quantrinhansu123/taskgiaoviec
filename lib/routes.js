...
export const ROUTES = {
  ...
  desktop: {
    ...
    attendanceList: `${DESKTOP_PREFIX}/attendance-list`,
    settings: `${DESKTOP_PREFIX}/cai-dat`,
  },
};
...
export function parseAppPath(pathname = '/') {
  ...
  if (head === 'attendance-list' || head === 'danh-sach-cham-cong' || head === 'bang-cong') {
    return { layout, tab: 'attendanceList', stack: [], personId: null };
  }
  if (head === 'cai-dat' || head === 'settings') {
    return { layout, tab: 'settings', stack: [], personId: null };
  }
...
export function pathForTab(tab, layout = 'mobile') {
  ...
  if (tab === 'attendanceList') return `${routePrefix(layout)}/attendance-list`;
  if (tab === 'settings') return `${routePrefix(layout)}/cai-dat`;
...
export function swapLayoutPath(pathname, targetLayout) {
  ...
  if (tab === 'attendanceList') return pathForTab('attendanceList', targetLayout);
  if (tab === 'settings') return pathForTab('settings', targetLayout);
  return pathForTab('products', targetLayout);
}

export const ROUTE_LABELS = [
  { path: ROUTES.products, label: 'Điện thoại — Dự án / Projects' },
  { path: ROUTES.subtasks, label: 'Điện thoại — Sub-task' },
  { path: ROUTES.people, label: 'Điện thoại — Nhân sự' },
  { path: ROUTES.attendance, label: 'Điện thoại — Chấm công' },
  { path: ROUTES.me, label: 'Điện thoại — Tôi' },
  { path: ROUTES.desktop.products, label: 'Máy tính — Dự án / Projects (full màn)' },
